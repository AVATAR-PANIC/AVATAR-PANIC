...
//	mapView.getController().set
	...
	//take the list of GeoPoints and enter that list into the cluster maker.
	...
	Map<MegaPoint, DataObject> tempMegaArray = ClusterMaker.pointClusterMaker(entriesColl, minPixelDistance, mapView);
	entriesColl.putAll((Map<? extends GeoPoint, ? extends DataObject>)tempMegaArray);
//	Map<GeoPoint, DataObject> tempMap = geoPointClusterMaker.pointClusterMaker(entriesColl, minPixelDistance, mapView);
	...
	for (GeoPoint entry : entriesColl.keySet()) {
	    ...
	    {
		//myCameraView.drawGeoPoint(myCameraView.getMyCanvas(), temporaryGeoPoint);
    ...
    public <E extends GeoPoint> void drawMarker(final E geoPoint, final DataObject dataObject)
    {
